...
export const useCrudForm = <T>(options: CrudFormApiOptions, entity: Ref<Partial<any>>) => {
  ...
  const onSubmit = async (): Promise<T | undefined> => {
    if (!entity.value || !apiEnpoint.value) {
      return
    }
    // A view screen is read-only. Without this guard the method resolves to PUT while
    // apiEnpoint resolves through the POST branch, sending PUT <collection> with no id.
    if (crudAction.value === CrudAction.VIEW) {
      return
    }
   return await onSubmitProcess<T>(
      entity.value,
      crudAction.value === CrudAction.EDIT ? (options.methodPut || 'PUT') : 'POST',
      apiEnpoint.value
    )
  }
  ...
  const onDelete = async () => {
    ...
    loading.value = true
    try {
      const response = await api<unknown>(deleteApiEndpoint.value, {
        method: 'DELETE'
      })
      if (import.meta.dev) {
