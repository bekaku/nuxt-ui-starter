...
export const useUpload = () => {
  ...
  const onUploadChunk = async (file: File, options?: {
    ...
    try {
      ...
      for (let chunkIndex = 0; chunkIndex < totalChunks; chunkIndex++) {
        ...
        for (let attempt = 0; attempt < maxRetries; attempt++) {
          try {
            ...
            const chunkResponse = await api.raw<FileUploadChunkResponse | FileManager | void>("/api/fileManager/uploadChunkApi", {
              method: "POST",
              body: formData,
            });
            if (chunkResponse && chunkResponse.status === 200) {
              chunkSuccess = true;
      ...
      let mergeData: FileUploadChunkMergeRequest = {
        chunkFilename: chunkFilename,
      ...
      if (options?.metaData) {
        if (options.metaData?.duration) mergeData.duration = options.metaData.duration;
        if (options.metaData?.hidden) mergeData.hidden = options.metaData.hidden;
      ...
      if (mergeResponse && mergeResponse.status === 200 && (mergeResponse as any)._data && (mergeResponse as any)._data.id) {
        responseFile = (mergeResponse as any)._data;
      } else {
